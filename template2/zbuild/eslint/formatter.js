...
var getFileLink = function(_path, line, column) {
    ...
    return scheme.replace('{file}', _path).replace('{line}', line).replace('{column}', column);
};
...
module.exports = function(results) {
    ...
    output += table(
            entries.reduce(function(seq, message) {
                ...
                filePath=path.relative(program.buildPath,filePath).replace(/\\/g,"/");
                ...
                seq.push([
                    "\t",
                    filename,
                    messageType + '  ' + message.message.replace(/\.$/, ''),
                ]);
                return seq;
            }, []), {
                align: [
