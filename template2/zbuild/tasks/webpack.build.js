...
gulp.task("checkBuildParam",function(cb){
    if(!program.file){
        console.info(new Error("must set the [-f,--file] param"));
        gulp.stop(null,true);
    }
    cb();
});
...
function printFileSizes(stats, previousSizeMap) {
    ...
    var assets = stats.toJson().assets
        ...
        .map(asset => {
            var fileContents = fs.readFileSync(outPath + '/' + asset.name);
...
function build(previousSizeMap,cb) {
    ...
    webpack(config).run((err, stats) => {
        ...
        var appPackage  = require(resolveApp("package.json"));
...
function copyPublicFolder() {
    const resolveApp=program.resolveApp;
    const outPath=program.outPath?resolveApp(program.outPath):resolveApp("build");
    fs.copySync( path.resolve(__dirname,"../template"), outPath, {
        dereference: true,
        filter: file => file !== path.resolve(__dirname,"../template/index.html")
    });
}