...
const crawlFilesystem = require('asar/lib/crawlfs')
const createSnapshot = require('asar/lib/snapshot')
        ...
        switch (file.type) {
            case 'directory':
                shouldUnpack = options.unpackDir
                    ...
                    : false
                filesystem.insertDirectory(file.vfilename||filename, shouldUnpack)
                break
            case 'file':
                shouldUnpack = false
                if (options.unpack) {
                ...
                filesystem.insertFile(file.vfilename||filename, shouldUnpack, file, options, done)
                return
            case 'link':
                filesystem.insertLink(file.vfilename||filename, file.stat)
                break
        }
